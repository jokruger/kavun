...
func runReuse(t *testing.T, cta *core.Arena, rta *core.Arena, src string, times int) []*kavun.Variable {
	t.Helper()
...
func runReuseSwitching(t *testing.T, cta *core.Arena, rta *core.Arena, scripts []string, rounds int) []*kavun.Variable {
	t.Helper()
...
func TestVMReuse_NamedResult_DefaultUndefinedAcrossRuns(t *testing.T) {
	...
	results := runReuse(t, cta, rta, src, 5)
	for i, r := range results {
		require.Equal(t, rta, true, r.Bool(rta), "run %d", i)
	}
}
...
func TestVMReuse_NamedResult_NoCrossScriptLeak(t *testing.T) {
	...
	cta := core.NewArena(nil)
	rta := core.NewArena(nil)
	out := runReuseSwitching(t, cta, rta, []string{scriptA, scriptB}, 3)
	for i := 0; i < len(out); i += 2 {
		require.Equal(t, rta, "from_A", out[i].String(rta), "round %d script A", i/2)
		require.Equal(t, rta, true, out[i+1].Bool(rta), "round %d script B", i/2)
	}
}
...
func TestVMReuse_Defer_NoLeakAcrossRuns(t *testing.T) {
	...
	cta := core.NewArena(nil)
	rta := core.NewArena(nil)
	results := runReuse(t, cta, rta, src, 4)
	for i, r := range results {
		require.Equal(t, rta, int64(1), r.Int(rta), "run %d: defer should fire exactly once per run", i)
	}
}
...
func TestVMReuse_Defer_MultipleAcrossRuns(t *testing.T) {
	...
	cta := core.NewArena(nil)
	rta := core.NewArena(nil)
	results := runReuse(t, cta, rta, src, 3)
	for i, r := range results {
		arr := r.Array(rta)
		require.Equal(t, rta, 3, len(arr), "run %d: should have exactly 3 defers per run", i)
		got := []string{}
		for _, e := range arr {
			got = append(got, e.(string))
		}
		require.Equal(t, rta, "c,b,a", strings.Join(got, ","), "run %d", i)
	}
}
...
func TestVMReuse_Recover_NoStaleErrorAcrossRuns(t *testing.T) {
	...
	results := runReuse(t, cta, rta, src, 4)
	for i, r := range results {
		require.Equal(t, rta, "no_error", r.String(rta), "run %d", i)
	}
}
...
func TestVMReuse_Recover_NoCrossScriptLeak(t *testing.T) {
	...
	cta := core.NewArena(nil)
	rta := core.NewArena(nil)
	out := runReuseSwitching(t, cta, rta, []string{scriptRaises, scriptClean}, 4)
	for i := 0; i < len(out); i += 2 {
		require.Equal(t, rta, "caught", out[i].String(rta), "round %d raises", i/2)
		require.Equal(t, rta, "ok", out[i+1].String(rta), "round %d clean", i/2)
	}
}
...
func TestVMReuse_DeferRecover_StressRepeat(t *testing.T) {
	...
	cta := core.NewArena(nil)
	rta := core.NewArena(nil)
	results := runReuse(t, cta, rta, src, 50)
	for i, r := range results {
		arr := r.Array(rta)
...
func TestVMReuse_NamedResult_WithTailCallAcrossRuns(t *testing.T) {
	...
	results := runReuse(t, cta, rta, src, 5)
	for i, r := range results {
		require.Equal(t, rta, "done", r.String(rta), "run %d", i)
	}
}
...
func TestVMReuse_Mixed_NamedDeferRecoverInterleaved(t *testing.T) {
	...
	cta := core.NewArena(nil)
	rta := core.NewArena(nil)
	out := runReuseSwitching(t, cta, rta, []string{scriptDeferRaise, scriptNamedOnly, scriptPlain}, 5)
	for i := 0; i < len(out); i += 3 {
		require.Equal(t, rta, "rescued", out[i].String(rta), "round %d deferRaise", i/3)
		require.Equal(t, rta, true, out[i+1].Bool(rta), "round %d namedOnly", i/3)
		require.Equal(t, rta, int64(6), out[i+2].Int(rta), "round %d plain", i/3)
	}
}
