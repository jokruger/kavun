...
func (c *Compiler) runSimplifyBooleanIdentities(node parser.Node) (parser.Node, bool, error) {
	fn := func(e parser.Expr) (parser.Expr, bool) {
		switch n := e.(type) {
		case *parser.UnaryExpr:
			// !true → false, !false → true
			if n.Token == token.Not {
				if bl, ok := n.Expr.(*parser.BoolLit); ok {
					return &parser.BoolLit{
						Value:    !bl.Value,
						ValuePos: n.TokenPos,
						Literal:  strconv.FormatBool(!bl.Value),
					}, true
				}
				// !!x → x  (only when x is provably bool)
				if inner, ok := n.Expr.(*parser.UnaryExpr); ok && inner.Token == token.Not {
					if isProvablyBool(inner.Expr) {
						return inner.Expr, true
					}
				}
			}
		case *parser.BinaryExpr:
			if n.Token != token.Equal && n.Token != token.NotEqual {
				return e, false
			}
			// Normalize: put bool literal on the RHS.
			var lhs parser.Expr
			var boolLit *parser.BoolLit
			if bl, ok := n.RHS.(*parser.BoolLit); ok {
				lhs = n.LHS
				boolLit = bl
			} else if bl, ok := n.LHS.(*parser.BoolLit); ok {
				lhs = n.RHS
				boolLit = bl
			} else {
				return e, false
			}
			if !isProvablyBool(lhs) {
				return e, false
			}
			switch {
			case n.Token == token.Equal && boolLit.Value:
				// x == true → x
				return lhs, true
			case n.Token == token.NotEqual && !boolLit.Value:
				// x != false → x
				return lhs, true
			case n.Token == token.Equal && !boolLit.Value:
				// x == false → !x
...
// runCopyPropagation replaces uses of a variable `y` that is initialized as a
// bare copy of another variable `x` — `y := x; use(y)` — with `x` at every use
// site. Safety requirements (all must hold):
...
//   - x is not `undefined`.
func (c *Compiler) runCopyPropagation(node parser.Node) (parser.Node, bool, error) {
	file, ok := node.(*parser.File)
	if !ok {
		return node, false, nil
	}
	...
	for _, s := range file.Stmts {
		...
		// x must be a real user variable (not a builtin function shadow) and
		// must be stable.
		...
		// x must be writable exactly once (its own declaration) and free of
		// closure capture that could rebind it later.
		if xU.writes != 1 || xU.insideFuncLit || xU.insideDefer || xU.addressed {
			continue
		}
		// x must not resolve to a builtin (builtins can be shadowed but we
		// avoid propagating names that could clash).
