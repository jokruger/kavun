...
func (c *Compiler) Compile(node parser.Node) (err error) {
	...
	case *parser.FuncLit:
		...
		if len(freeSymbols) > 0 {
			_, err = c.emit(node, bc.OpClosure, c.addConstant(cf), len(freeSymbols))
			if err != nil {
				return err
			}
		} else {
			_, err = c.emit(node, bc.OpConstant, c.addConstant(cf))
			if err != nil {
				return err
			}
		}
	...
	case *parser.ImportExpr:
		...
		if mod, ok := stdlib.GetModuleID(node.ModuleName); ok { // builtin module
			...
			_, err = c.emit(node, bc.OpConstant, c.addConstant(compiled))
			if err != nil {
				return err
			}
