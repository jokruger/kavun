...
func testBinaryOp(t *testing.T, lhs core.Value, op token.Token, rhs core.Value, expected core.Value) {
	...
	require.Equal(t, a, expected, actual)
}
...
func TestObject_Value(t *testing.T) {
	...
	require.NoError(t, err)
	v, err = rta.NewErrorValue(x, core.KindUser, false)
	require.NoError(t, err)
	require.True(t, v.Type == value.Error)
	bs, err = v.EncodeBinary(rta)
	require.NoError(t, err)
	...
	v, err = rta.NewIntRangeValue(0, 10, 2)
	require.NoError(t, err)
	require.True(t, v.Type == value.IntRange)
	rng = rta.ResolveIntRangeValue(v)
...
func TestObject_IsTrue(t *testing.T) {
	...
	require.NoError(t, err)
	require.False(t, o.IsTrue(rta))
	o, err = rta.NewArrayValue([]core.Value{core.Undefined}, false)
	require.NoError(t, err)
	require.True(t, o.IsTrue(rta))
	...
	require.NoError(t, err)
	require.False(t, o.IsTrue(rta))
	o, err = rta.NewRecordValue(map[string]core.Value{"a": core.Undefined}, false)
	require.NoError(t, err)
	require.True(t, o.IsTrue(rta))
	...
	require.NoError(t, err)
	require.False(t, o.IsTrue(rta))
	o, err = rta.NewBytesValue([]byte{1, 2}, false)
	require.NoError(t, err)
	require.True(t, o.IsTrue(rta))

	// empty range is false, non-empty range is true
	o, err = rta.NewIntRangeValue(0, 0, 1)
	require.NoError(t, err)
	require.False(t, o.IsTrue(rta))
	o, err = rta.NewIntRangeValue(0, 10, 1)
	require.NoError(t, err)
	require.True(t, o.IsTrue(rta))
