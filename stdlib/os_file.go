...
func makeOSFile(vm core.VM, file *os.File) *value.Record {
	...
	fileName := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		if len(s) > core.MaxStringLen {
			return core.UndefinedValue(), core.NewStringLimitError("os.file.name")
		}
	...
	fileChown := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.chown", "first", "int(compatible)", args[0].TypeName())
		}
		i2, ok := args[1].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.chown", "second", "int(compatible)", args[1].TypeName())
		}
	...
	fileWrite := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.write", "first", "bytes(compatible)", args[0].TypeName())
		}
	...
	fileRead := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		y1, ok := args[0].AsBytes()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.read", "first", "bytes(compatible)", args[0].TypeName())
		}
	...
	fileWriteString := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.write_string", "first", "string(compatible)", args[0].TypeName())
		}
	...
	fileReaddirnames := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.read_dir_names", "first", "int(compatible)", args[0].TypeName())
		}
		...
		for _, r := range res {
			if len(r) > core.MaxStringLen {
				return core.UndefinedValue(), core.NewStringLimitError("os.file.read_dir_names")
			}
	...
	fileChmod := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.chmod", "first", "int(compatible)", args[0].TypeName())
		}
	...
	fileSeek := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.seek", "first", "int(compatible)", args[0].TypeName())
		}
		i2, ok := args[1].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("os.file.seek", "second", "int(compatible)", args[1].TypeName())
		}
