...
func randPerm(vm core.VM, args []core.Value) (core.Value, error) {
	...
	i1, ok := args[0].AsInt()
	if !ok {
		return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.perm", "first", "int(compatible)", args[0].TypeName())
	}
...
func randSeed(vm core.VM, args []core.Value) (core.Value, error) {
	...
	i1, ok := args[0].AsInt()
	if !ok {
		return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.seed", "first", "int(compatible)", args[0].TypeName())
	}
	rand.Seed(i1)
...
func randInt63n(vm core.VM, args []core.Value) (core.Value, error) {
	...
	i1, ok := args[0].AsInt()
	if !ok {
		return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.int_n", "first", "int(compatible)", args[0].TypeName())
	}
...
func randRead(vm core.VM, args []core.Value) (core.Value, error) {
	...
	if !ok {
		return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.read", "first", "bytes", args[0].TypeName())
	}
...
func randFunc(vm core.VM, args []core.Value) (core.Value, error) {
	...
	i1, ok := args[0].AsInt()
	if !ok {
		return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.rand", "first", "int(compatible)", args[0].TypeName())
	}
...
func randRand(vm core.VM, r *rand.Rand) *value.Record {
	...
	rRead := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		y1, ok := args[0].AsBytes()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.rand.read", "first", "bytes", args[0].TypeName())
		}
		res, err := r.Read(y1)
		if err != nil {
			return wrapError(vm, err), nil
	...
	rInt63n := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.rand.int_n", "first", "int(compatible)", args[0].TypeName())
		}
	...
	rSeed := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.rand.seed", "first", "int(compatible)", args[0].TypeName())
		}
		r.Seed(i1)
		return core.UndefinedValue(), nil
	}
	...
	rPerm := func(vm core.VM, args []core.Value) (core.Value, error) {
		...
		i1, ok := args[0].AsInt()
		if !ok {
			return core.UndefinedValue(), core.NewInvalidArgumentTypeError("rand.rand.perm", "first", "int(compatible)", args[0].TypeName())
		}
		res := r.Perm(int(i1))
		arr := make([]core.Value, 0, len(res))
		alloc := vm.Allocator()
